...
- ✅ **K10 端配套**（固件 v3.2）：环境报告到达后显示**多玩家列表页**（名字+温度+湿度，最多8行+剩余计数），约 8 秒后**自动返回城市大屏**
...
- 🎯 **三大显示模式** - 按 K10 B键自由切换（聊天/环境/城市仪表盘）
...
**v1.2.0 界面逻辑（固件 v3.2）：**
- 开机默认显示**城市大屏**；任何界面 30 秒无操作自动返回城市大屏
- 环境聚合报告到达时临时显示**多玩家环境列表**约 8 秒，自动返回城市大屏
- 玩家加入/阵亡/住户变动/红石激增等事件全屏弹出通知约 5 秒后恢复
